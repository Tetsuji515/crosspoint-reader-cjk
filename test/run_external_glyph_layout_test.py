...
TEST_FILE = REPO_ROOT / "test" / "ExternalGlyphLayoutTest.cpp"
LAYOUT_CPP = REPO_ROOT / "lib" / "GfxRenderer" / "ExternalGlyphLayout.cpp"
EXTERNAL_FONT_CPP = REPO_ROOT / "lib" / "ExternalFont" / "ExternalFont.cpp"
HOST_STUBS = REPO_ROOT / "test" / "support" / "host_stubs"
LAYOUT_INCLUDE = REPO_ROOT / "lib" / "GfxRenderer"
EXTERNAL_FONT_INCLUDE = REPO_ROOT / "lib" / "ExternalFont"

FAIL_STUB = REPO_ROOT / "test" / "support" / "external_glyph_layout_task3_fail_stub.cpp"
...
def main() -> int:
    ...
    if mode not in {"pass", "fail"}:
        print("Usage: test/run_external_glyph_layout_test.py [pass|fail]", file=sys.stderr)
        return 2

    with tempfile.TemporaryDirectory(prefix="external-glyph-layout-test-") as temp_dir_str:
        temp_dir = Path(temp_dir_str)
        binary = temp_dir / "ExternalGlyphLayoutTest"
        ...
        result = compile_test([TEST_FILE, LAYOUT_CPP, EXTERNAL_FONT_CPP], binary)
        print_result(result)
        if result.returncode != 0:
            return result.returncode
