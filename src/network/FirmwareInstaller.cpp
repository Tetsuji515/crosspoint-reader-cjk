...
FirmwareInstaller::Error FirmwareInstaller::validateImageHeader(HalFile& file) {
  uint8_t headerBuf[kImageHeaderCheckSize];
  const int bytesRead = file.read(headerBuf, sizeof(headerBuf));
  if (bytesRead < static_cast<int>(sizeof(esp_image_header_t))) {
    LOG_ERR("FW_INSTALL", "Failed to read image header");
    return Error::READ_ERROR;
  }

  esp_image_header_t imageHeader;
  memcpy(&imageHeader, headerBuf, sizeof(imageHeader));
